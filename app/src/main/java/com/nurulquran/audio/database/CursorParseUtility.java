...
public class CursorParseUtility {
	...
	public static String getString(Cursor row, String columName) {
		if (row == null || columName == null || columName.equalsIgnoreCase("") || row.getColumnIndex(columName) == -1) {
			Logger.d(TAG, "No ColumnName found :" + columName);
			return "";
		} else {

			return row.getString(row.getColumnIndex(columName)) != null ? row.getString(row.getColumnIndex(columName))
					: "";
		}
	}
	...
	public static double getDouble(Cursor row, String columName) {
		if (row == null || columName == null || columName.equalsIgnoreCase("") || row.getColumnIndex(columName) == -1) {
			...
			return row.getColumnIndex(columName) != -1 ? row.getDouble(row.getColumnIndex(columName)) : 0.0;
		}
	}
	...
	public static int getInt(Cursor row, String columName) {
		if (row == null || columName == null || columName.equalsIgnoreCase("") || row.getColumnIndex(columName) == -1) {
			...
			return row.getColumnIndex(columName) != -1 ? row.getInt(row.getColumnIndex(columName)) : 0;
		}
